...
namespace Gamekit2D
{
    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        public void LoseShield()
        {
            ...
            StartCoroutine(invulnerabilityFromShield());
        }

        public void GainShield()
        {
            var d = GetComponent<Damageable>();
            d.EnableInvulnerability();
            shield.SetActive(true);
        }

        IEnumerator invulnerabilityFromShield()
        {
            yield return new WaitForSeconds(5.0f); //wait one second before respawing
            var d = GetComponent<Damageable>();
            d.DisableInvulnerability();
        }
    }
}
